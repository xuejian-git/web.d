...
AnalysisState HttpData::analysisRequest() {
    ...
    if (method_ == METHOD_POST) {
        ...
        /*
           ...
           header += "Server: Apriluestc's webd\r\n";
...
// void HttpData::handleDynamic() {}

int HttpData::sendPhpToCli(int fd, int outlen, char* out, int errlen, char* err, FastCgiEndRequestBody* endr) {
    (void)endr;
    char *p; 
    int n;
    char buf[MAXLINE];
   ...
   int HttpData::recvFastCgi(int fd, int fcsock) {
   int requestId;
   requestId = fcsock;
   FastCgiData fcd;
   if (fcd.RecvRecord(rio_readn, sendPhpToCli, fd, fcsock, requestId) < 0) {
   return -1;
   }
   return 0;
   }
